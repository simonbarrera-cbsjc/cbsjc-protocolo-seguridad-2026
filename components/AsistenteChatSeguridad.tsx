    ...
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen && (
        <button
          ...
          className="group relative flex h-14 w-14 items-center justify-center rounded-full bg-primary text-white shadow-[0_10px_25px_rgba(11,25,83,0.3)] transition-all duration-300 hover:scale-105 hover:bg-primary-light active:scale-95 cursor-pointer border-2 border-white"
          aria-label="Abrir asistente de IA de Seguridad"
        >
          <ShieldCheck className="h-7 w-7 transition-transform group-hover:scale-110 text-amber-300" />
      ...
      {isOpen && (
        <div className="flex h-[560px] w-[350px] sm:w-[410px] flex-col rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(11,25,83,0.18)] overflow-hidden animate-slide-up">
          {/* Header (Light/Primary Institutional) */}
          <div className="flex items-center justify-between bg-primary p-4 text-white border-b border-primary/20">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/10 text-amber-300 border border-white/20 shadow-xs">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-blue-100 font-medium">Protocolos SJB-RGD003 V2 • IA</span>
              </div>
            </div>
          ...
          {/* Messages Feed (100% Light Theme Plain Text) */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-50 custom-scrollbar">
            {messages.map((m, idx) => (
              ...
              >
                {m.role === "assistant" && (
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary border border-primary/20 mt-1 shadow-xs">
                ...
                <div
                  className={`max-w-[84%] rounded-2xl p-3.5 text-xs leading-relaxed ${
                    m.role === "user"
                      ? "bg-primary text-white font-semibold shadow-xs rounded-br-xs"
                      : "bg-white text-slate-800 border border-slate-200 shadow-xs rounded-tl-xs whitespace-pre-line font-medium"
                  }`}
                >
                  {cleanPlainText(m.content)}
                </div>

                {m.role === "user" && (
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-accent/10 text-accent border border-accent/20 mt-1 shadow-xs">
            ...
            {loading && (
              <div className="flex gap-2.5 justify-start items-center">
                ...
                <div className="bg-white border border-slate-200 p-3 rounded-2xl shadow-xs text-xs text-slate-600 flex items-center gap-2 font-semibold">
              ...
              <div className="flex flex-col gap-1">
                {FAQ_SUGGESTIONS.slice(0, 3).map((faq, fIdx) => (
                  <button
                    ...
                    className="text-left text-[11px] font-bold text-primary hover:text-accent truncate py-1 px-2 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer"
                  >
                    • {faq}
                  </button>
                ))}
          ...
          <div className="p-3 bg-white border-t border-slate-200">
            ...
            >
              <input
                ...
                className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-800 placeholder-slate-400 focus:border-primary focus:bg-white focus:outline-none transition-all"
              />
              <button
                type="submit"
                disabled={!input.trim() || loading}
                className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent text-white hover:bg-accent-dark disabled:opacity-40 disabled:hover:bg-accent transition-all cursor-pointer shrink-0 shadow-xs"
              >
